...
export default function ChatHistorySidebar({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800/80 backdrop-blur-xl overflow-hidden shadow-sm dark:shadow-xl">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800/80 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-500 dark:text-slate-400 font-mono bg-slate-100 dark:bg-slate-800/80 px-2 py-0.5 rounded-full">
        ...
        {sessions.length > 3 && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 rounded-lg text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-sky-500/50 transition-colors"
            />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1 space-y-1.5 custom-scrollbar">
        {filteredSessions.length === 0 ? (
          <div className="py-8 text-center px-4">
            <MessageSquareDashed className="w-8 h-8 mx-auto text-slate-300 dark:text-slate-600 mb-2" />
            ...
            return (
              <div
                ...
                className={`group relative flex items-center justify-between p-2.5 rounded-xl cursor-pointer transition-all duration-150 text-xs ${
                  isActive
                    ? "bg-sky-50 dark:bg-sky-500/15 border border-sky-200 dark:border-sky-500/30 text-sky-900 dark:text-sky-200 shadow-xs"
                    : "hover:bg-slate-100 dark:hover:bg-slate-800/60 text-slate-700 dark:text-slate-300 border border-transparent hover:border-slate-200 dark:hover:border-slate-800"
                }`}
              >
