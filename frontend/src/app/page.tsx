...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050814] text-slate-100 flex flex-col font-sans selection:bg-indigo-500/30 selection:text-indigo-200">
      {/* Background Neon Glow Elements */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[-10%] left-[15%] w-[500px] h-[500px] bg-indigo-600/10 rounded-full blur-[140px]" />
        <div className="absolute bottom-[-10%] right-[10%] w-[600px] h-[600px] bg-sky-600/10 rounded-full blur-[160px]" />
      </div>

      {/* Top Header & Navigation Bar */}
      <header className="sticky top-0 z-50 border-b border-slate-800/80 bg-[#050814]/90 backdrop-blur-xl px-6 py-3 shadow-xl shadow-black/30">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-base font-bold tracking-tight bg-gradient-to-r from-white via-indigo-100 to-sky-200 bg-clip-text text-transparent">
                  LexiGuard AI
                </h1>
                <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-md bg-indigo-500/15 text-indigo-300 border border-indigo-500/30">
                  v2.5 Workspace
                </span>
              </div>
              <p className="text-[11px] text-slate-400">Enterprise Legal Contract Intelligence & Risk Auditor</p>
            </div>
          </div>

          {/* Navigation Tabs */}
          <nav className="flex items-center p-1.5 rounded-2xl bg-slate-900/80 border border-slate-800/90 shadow-inner backdrop-blur-md">
            <button
              onClick={() => setActiveTab("overview")}
              className={`flex items-center gap-2 py-2 px-3.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                activeTab === "overview"
                  ? "bg-gradient-to-r from-indigo-900/90 to-slate-800 text-indigo-200 border border-indigo-500/40 shadow-sm"
      ...
      <footer className="relative z-10 border-t border-slate-800/60 bg-slate-950/40 py-4 px-6 text-center text-xs text-slate-400">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="text-slate-500 font-medium">Built for High-Impact Legal Due Diligence & Document Reasoning</span>
        </div>
      </footer>
    </div>
  );
